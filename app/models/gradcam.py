...
class GradCam():
    ...
    def plot(self,plot_hm=True,plot_gbp=True):
        ...
        col=0
        label0_title = f'{self.label1} {self.prob1:.3f}'
        size=self.xb_img.shape[-1]
        self.xb_img.show(row_axes[col])
        row_axes[col].set_title(label0_title)
        col+=1
        ...
        bytesIO_obj = BytesIO()
        fig.savefig(bytesIO_obj, format='png')
        bytesIO_obj.seek(0)

        img_uri = "data:image/png;base64," + base64.b64encode(bytesIO_obj.read()).decode('utf8')
